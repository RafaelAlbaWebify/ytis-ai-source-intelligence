    ...
    with ui.row().classes("w-full text-white ytis-shell"):
        ...
        with ui.column().classes("ytis-main p-5 gap-3 flex-1"):
            ...
            with ui.row().classes("w-full gap-4 items-stretch"):
                with ui.card().classes("ytis-card p-3 gap-1").style("flex: 1.18; min-width: 430px; height: 430px; overflow: hidden;"):
                ...
                with ui.card().classes("ytis-card p-3 gap-1").style("flex: 1.06; min-width: 360px; height: 430px; overflow: hidden;"):
                    with ui.row().classes("w-full justify-between items-center"):
                        ui.label("Process Log").classes("text-xl font-bold")
                        process_status = ui.label("Waiting").classes("text-sm text-green-500")
                    process_log = ui.log(max_lines=300).classes("w-full ytis-process-log").style("height: 352px;")

                with ui.card().classes("ytis-card p-3 gap-1").style("flex: 0.96; min-width: 330px; height: 430px; overflow: hidden;"):
                    with ui.row().classes("w-full justify-between items-center"):
                        ui.label("Source Preview").classes("text-xl font-bold")
                        source_type = ui.label("Channel").classes("text-xs text-red-400")
                    source_image = ui.image(SOURCE_PLACEHOLDER).classes("w-full rounded-xl").style("height: 205px; object-fit: cover;")
            ...
            if project_summaries:
                load_project(project_summaries[0])
                metric_labels["videos"].text = str(project_summaries[0].get("videos_found", "-"))
                metric_labels["subtitles"].text = str(project_summaries[0].get("transcripts_created", "-"))
                metric_labels["txt"].text = str(project_summaries[0].get("transcripts_created", "-"))
                total_words_value = project_summaries[0].get("total_words", "")
                metric_labels["words"].text = f"{int(total_words_value):,}" if str(total_words_value).isdigit() else "-"
                metric_labels["zip"].text = "1" if project_summaries[0].get("zip_path") else "-"
                for label in metric_labels.values():
                    label.update()
            else:
                update_source_preview()
