...
def parse_filename_metadata(path: Path) -> tuple[str, str, str]:
    """Parse upload date, title, and video ID from yt-dlp output filenames.

    This parser deliberately takes the title from the text before the [video_id]
    block. That prevents subtitle suffixes such as .en-en-XXXX from polluting titles.
    """
    stem = path.stem
...
def clean_all_srt(raw_srt_dir: Path, clean_txt_dir: Path, callback: LogCallback | None = None) -> list[TranscriptRecord]:
    ...
    # Clean previous TXT files so rebuilds do not leave stale duplicates.
