...
def index() -> None:
    ...
    with ui.row().classes("w-full text-white ytis-shell"):
        with ui.column().classes("ytis-sidebar p-3 gap-2"):
        ...
        with ui.column().classes("ytis-main p-4 gap-3 flex-1"):
            ...
            with ui.row().classes("w-full gap-4 items-stretch"):
                with ui.card().classes("ytis-card p-3 gap-1").style("flex: 1.08; min-width: 420px; height: 450px; overflow: hidden;"):
                ...
                with ui.card().classes("ytis-card p-3 gap-1").style("flex: 1.05; min-width: 360px; height: 450px; overflow: hidden;"):
                    with ui.row().classes("w-full justify-between items-center"):
                        ui.label("Process Log").classes("text-xl font-bold")
                        process_status = ui.label("Waiting").classes("text-sm text-green-500")
                    process_log = ui.log(max_lines=300).classes("w-full ytis-process-log").style("height: 373px;")

                with ui.card().classes("ytis-card p-3 gap-1").style("flex: 1.2; min-width: 420px; height: 450px; overflow: hidden;"):
                    with ui.row().classes("w-full justify-between items-center"):
                        ui.label("Source Preview").classes("text-xl font-bold")
                        source_type = ui.label("Channel").classes("text-xs text-red-400")
                    source_image = ui.image(SOURCE_PLACEHOLDER).classes("w-full rounded-xl").style("height: 210px; object-fit: cover;")
