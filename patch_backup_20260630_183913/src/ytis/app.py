...
def index() -> None:
    ...
    with ui.row().classes("w-full text-white ytis-shell"):
        with ui.column().classes("ytis-sidebar p-4 gap-3"):
            ...
            current_project_subtitle = ui.label("Current project").classes("text-xs text-slate-400")
            ...
            with ui.card().classes("ytis-mini-card p-3 w-full"):
                ...
                with ui.row().classes("items-center gap-3 mt-2"):
                    ui.avatar(icon="person").classes("bg-red-500")
                    with ui.column().classes("gap-0"):
                ...
                with ui.column().classes("gap-2"):
                    with ui.row().classes("items-center gap-2"):
                        ui.icon("sync").classes("text-blue-400")
                        current_mode
            ...
            with ui.card().classes("ytis-mini-card p-3 w-full"):
                ...
                quick_new_project_button = ui.button("New Project", icon="add", color="primary").classes("w-full mt-2").props("dense")
                quick_load_last_button = ui.button("Load Last Project", icon="bolt").classes("w-full").props("outline dense")
                quick_open_downloads_button = ui.button("Open Downloads Folder", icon="folder_open").classes("w-full").props("outline dense")
                quick_open_zip_button = ui.button("Open Last ZIP", icon="inventory_2").classes("w-full").props("outline dense")
                quick_open_project_button = ui.button("Open Project Folder", icon="folder").classes("w-full").props("outline dense")
            ...
            with ui.card().classes("ytis-mini-card p-3 w-full"):
                ...
                ui.label("System Ready").classes("text-green-400 text-sm mt-2")
        ...
        with ui.column().classes("ytis-main p-5 gap-3 flex-1"):
            ...
            with ui.row().classes("w-full gap-4 items-stretch"):
                with ui.card().classes("ytis-card p-3 gap-1").style("flex: 1.08; min-width: 420px; height: 455px; overflow: hidden;"):
                ...
                with ui.card().classes("ytis-card p-3 gap-1").style("flex: 1.05; min-width: 360px; height: 455px; overflow: hidden;"):
                    with ui.row().classes("w-full justify-between items-center"):
                        ui.label("Process Log").classes("text-xl font-bold")
                        process_status = ui.label("Waiting").classes("text-sm text-green-500")
                    process_log = ui.log(max_lines=300).classes("w-full ytis-process-log").style("height: 378px;")

                with ui.card().classes("ytis-card p-3 gap-1").style("flex: 1.2; min-width: 420px; height: 455px; overflow: hidden;"):
                    with ui.row().classes("w-full justify-between items-center"):
                        ui.label("Source Preview").classes("text-xl font-bold")
                        source_type = ui.label("Channel").classes("text-xs text-red-400")
                    source_image = ui.image(SOURCE_PLACEHOLDER).classes("w-full rounded-xl").style("height: 230px; object-fit: cover;")
