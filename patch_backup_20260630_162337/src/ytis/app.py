...
def dashboard() -> None:
    ...
    with ui.row().classes("w-full min-h-screen text-white"):
        with ui.column().classes("ytis-sidebar p-5 gap-4 w-72"):
            with ui.row().classes("items-center gap-3"):
                ...
                with ui.column().classes("gap-0"):
                    ...
                    ui.label("v0.1.2").classes("text-xs text-slate-500")
        ...
        with ui.column().classes("p-8 gap-5 flex-1") as main_col:
            ...
            with ui.card().classes("ytis-card p-6 w-full") as build_card:
                ...
                def reset_form() -> None:
                    ...
                    CURRENT_LOG.clear()
                    for element in [name, url, lang, output, progress, status]:
                        element.update()
                ...
                def load_project(project: dict) -> None:
                    ...
                    output.value = str(default_downloads_dir())
                    for element in [name, url, lang, output]:
                        element.update()
                                    ...
                                    with ui.row().classes("gap-2"):
                                        ...
                                        if project.get("project_dir"):
                                            ui.button(
                                                "Open Folder",
                                                icon="folder_open",
                                                on_click=lambda p=project: os.startfile(str(p.get("project_dir"))),
                                            ).props("outline")
