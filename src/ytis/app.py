...
def index() -> None:
    ...
    .q-field { padding-bottom: 0 !important; margin-bottom: 0 !important; }
    .q-field__control { min-height: 30px !important; height: 30px !important; }
    .q-field__label { font-size: 10px !important; top: 8px !important; }
    .q-field__native { font-size: 11px !important; min-height: 30px !important; }
    .q-btn { min-height: 28px !important; padding: 4px 8px !important; }
    .ytis-sidebar .q-btn { min-height: 28px !important; }
    .ytis-main .q-card { box-shadow: 0 8px 24px rgba(0,0,0,0.20); }
    .ytis-process-log { font-size: 11px !important; }
    ...
    with ui.row().classes("w-full text-white ytis-shell"):
        with ui.column().classes("ytis-sidebar p-2 gap-1"):
            with ui.row().classes("items-center gap-3"):
                ui.icon("smart_display").classes("text-4xl text-red-500")
                with ui.column().classes("gap-0"):
                    ui.label("YTIS").classes("text-2xl font-bold text-blue-400")
        ...
        with ui.column().classes("ytis-main p-3 gap-2 flex-1"):
            with ui.row().classes("w-full justify-between items-center"):
                with ui.column().classes("gap-0"):
                    ui.label("Dashboard").classes("text-2xl font-bold")
                    ui.label("Build upload-ready YouTube research packs").classes("text-xs text-slate-300")
            ...
            with ui.row().classes("w-full gap-4 items-stretch"):
                with ui.card().classes("ytis-card p-2 gap-0").style("flex: 1.08; min-width: 420px; height: 365px; overflow: hidden;"):
                    ui.label("Build Research Pack").classes("text-lg font-bold")
                    ...
                    ui.label("reuse = existing subtitles | refresh = YouTube | repackage = ZIP only").classes("text-[10px] text-slate-400")
                ...
                with ui.card().classes("ytis-card p-2 gap-0").style("flex: 1.05; min-width: 360px; height: 365px; overflow: hidden;"):
                    with ui.row().classes("w-full justify-between items-center"):
                        ui.label("Process Log").classes("text-lg font-bold")
                        process_status = ui.label("Waiting").classes("text-sm text-green-500")
                    process_log = ui.log(max_lines=300).classes("w-full ytis-process-log").style("height: 296px;")

                with ui.card().classes("ytis-card p-2 gap-0").style("flex: 1.2; min-width: 420px; height: 365px; overflow: hidden;"):
                    with ui.row().classes("w-full justify-between items-center"):
                        ui.label("Source Preview").classes("text-lg font-bold")
                        source_type = ui.label("Channel").classes("text-xs text-red-400")
                    source_image = ui.image(SOURCE_PLACEHOLDER).classes("w-full rounded-xl").style("height: 150px; object-fit: cover;")
                    source_title = ui.label("No source selected").classes("text-sm font-bold")
                    source_subtitle = ui.label("Enter a YouTube channel or video URL").classes("text-xs text-slate-400")
                    source_status = ui.label("Preview area ready.").classes("text-[10px] text-slate-400")
            ...
            with ui.row().classes("w-full gap-3"):
                for title, key, default in [
                    ...
                    with ui.card().classes("ytis-metric p-3").style("flex: 1; min-width: 170px; height: 70px;"):
                        ui.label(title).classes("text-sm text-slate-400")
                        metric_labels[key] = ui.label(default).classes("text-xl font-bold")

            with ui.row().classes("w-full gap-4 items-stretch"):
                with ui.card().classes("ytis-card p-3 gap-1").style("flex: 1.08; min-width: 500px; height: 185px; overflow: hidden;"):
                    ui.label("Last Build Summary").classes("text-base font-bold")
                ...
                with ui.card().classes("ytis-card p-3 gap-1").style("flex: 1; min-width: 520px; height: 185px; overflow: hidden;"):
                    with ui.row().classes("w-full justify-between items-center"):
                        ui.label("Recent Projects").classes("text-base font-bold")
            ...
            ui.timer(0.2, lambda: ui.run_javascript("window.scrollTo({top: 0, behavior: 'auto'});"), once=True)
